...
function generateSignal(symbol, price, rsi) {
  ...
  return {
    ...
    shares: positionSize,
    quality: rsi < 15 || rsi > 85 ? 'HIGH' : 'MEDIUM',
...
app.get('/api/export/csv', async (req, res) => {
  const signals = await fetchMarketData();
  const csv = 'Symbol,Price,RSI,Signal,Entry,StopLoss,TakeProfit,Shares,Quality,Status,Timestamp\\n' +
    signals.map(s => `${s.symbol},${s.price},${s.rsi},${s.signal},${s.entryPrice},${s.stopLoss},${s.takeProfit},${s.shares},${s.quality},${s.status},${s.timestamp}`).join('\\n');
